...
from incident_classifier import LLM_MODEL_DEFAULT
from prompt_eval import build_test_data, run_eval, score_output


def test_score_output_perfect_match():
    generated = (
        "aaiid_data_source,entry_id,json_blob,reasoning,confidence,llm_meta\n"
        'nhtsa_incident_report,e0,"{""Report ID"":""A1""}",the duck,High,{}\n'
    )
    result = score_output(generated, {"A1"})
    assert result["precision"] == 1.0
    assert result["recall"] == 1.0
    assert result["f1"] == 1.0


def test_score_output_empty_csv():
    generated = "aaiid_data_source,entry_id,json_blob,reasoning,confidence,llm_meta\n"
    result = score_output(generated, {"A1"})
